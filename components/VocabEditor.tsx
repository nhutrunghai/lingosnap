...
const VocabEditor: React.FC<VocabEditorProps> = ({ initialList, onSave, onCancel }) => {
  ...
  return (
    <div className="mx-auto max-w-4xl rounded-3xl border border-gray-100 bg-white p-5 shadow-2xl animate-slideUp">
      <div className="flex flex-col justify-between items-start mb-8 gap-4 sm:flex-row sm:items-center">
        ...
        <button onClick={handleAddNew} className="flex items-center rounded-2xl bg-blue-600 px-6 py-3 text-sm font-black text-white hover:bg-blue-700 shadow-lg shadow-blue-200">
      ...
      <div className="space-y-6 mb-8 max-h-[60vh] overflow-y-auto px-1">
        {list.length === 0 ? (
          ...
          list.map((item, index) => (
            <div key={item.id} className="p-5 bg-white rounded-3xl border border-gray-100 shadow-sm space-y-4 hover:border-blue-200 transition">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className="space-y-1.5 text-xs font-bold text-gray-400">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-2xl bg-gray-50 text-xs font-bold text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-blue-500"
                ...
                <label className="space-y-1.5 text-xs font-bold text-gray-400">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-2xl bg-green-50 text-xs font-bold text-green-700 outline-none focus:bg-white focus:ring-2 focus:ring-green-500"
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-gray-100">
        <button onClick={onCancel} className="px-6 py-3 rounded-2xl text-slate-500 font-bold hover:bg-gray-100 transition order-2 sm:order-1">
          Hủy bỏ
        </button>
        <button onClick={() => onSave(list)} className="px-10 py-3 rounded-2xl bg-slate-950 text-white font-black hover:bg-blue-600 shadow-lg active:scale-95 order-1 sm:order-2">
